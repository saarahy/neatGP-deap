...
#    direccion="./data_corridas/EnergyHeating/energy_efficiency_Heat.txt"
    ...
    np.savetxt('./data_corridas/Concrete/test_%d_%d.txt'%(p,n_corr), data_test, delimiter=",", fmt="%s")
    np.savetxt('./data_corridas/Concrete/train_%d_%d.txt'%(p,n_corr), data_train, delimiter=",", fmt="%s")
...
def main(n_corr, p):
    direccion="./data_corridas/Concrete/ConcreteData.txt"
    ...
    outfile = open('./Results/Concrete/popfinal_%d_%d.txt' % (p, n_corr), 'w')
