...
def evalSymbReg(individual, points):
    ...
    values = numpy.sin(points[:]**2)*numpy.cos(points[:])-1
...
# def evalSymbReg(individual, points):
#     func = toolbox.compile(expr=individual)
#     sqerrors = ((func(x) - (math.sin(x**2)*math.cos(x)-1))**2 for x in points)
#     return math.fsum(sqerrors) / len(points),
#
#
# def Nguyen5(n_corr):
#     with open("./data_corridas/Nguyen5/corrida%d/test_x.txt" % n_corr) as spambase:
#         spamReader = csv.reader(spambase)
#         spam = [float(row[0]) for row in spamReader]
#     with open("./data_corridas/Nguyen5/corrida%d/train_x.txt"% n_corr) as spamb:
...
if __name__ == "__main__":
    ...
    while n <= 30:
        #main(n, 3)
        cProfile.run('print main(n, 3); print')
        n += 1
